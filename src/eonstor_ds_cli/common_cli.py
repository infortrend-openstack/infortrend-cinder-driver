...
class InfortrendCommon(object):
    ...
    def _check_initiator_has_lun_map(self, initiator_info, map_info):
        if isinstance(initiator_info, list):
            for initiator_wwn in initiator_info:
                for entry in map_info:
                    if initiator_wwn.lower() == entry['Host-ID'].lower():
                        return True
        else:
            for entry in map_info:
                if initiator_info.lower() == entry['Host-ID'].lower():
                    return True
        return False
